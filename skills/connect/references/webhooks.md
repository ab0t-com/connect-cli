# Webhooks — receive, verify, retry

Provider event → integration receiver → matched to your subscriptions → **signed HTTP POST to your endpoint** → retried on failure → observable via deliveries/`tail`.
...
- [Real-time stream (tail)](#real-time-stream-tail)
...
- Receiver URL shape: `{api_url}/v2/webhooks/v1/receiver/{service_id}/{connection_id}/events` — paste into the provider's webhook settings.
...
Every delivery ab0t POSTs to your endpoint carries:
```
...
connect webhooks deliveries stats # success/fail counts
connect webhooks events list # raw inbound events
```
- `--output json` on any list for scripting/agents.
- `deliveries show <id>` is a point-read by `del_<hex>` id.

## Real-time stream (tail)
```bash
connect tail # live WS stream of events + delivery status
```
Caveat: real-time streaming pushes via the service's in-process delivery path. In **lambda-delivery mode** (current prod), the lambda does not publish to the WS channel, so `tail` connects but may show no live delivery updates — use `deliveries list` to confirm. (Tracked as a known infra gap.)
